...
class UIFriendlyLogger:
    ...
    def log_list_dict_as_result(self, results: List[dict]):
        ...
        try:
            ...
            keys: List[str] = []
            if hasattr(first_result, "keys") and callable(
                getattr(first_result, "keys")
            ):
                keys.extend(first_result.keys())
            elif hasattr(first_result, "model_fields_set"):
                keys.extend(first_result.model_fields_set)
            for key in keys:
                html += f"<th>{key}</th>"
            html += "</tr>"
            for row in results:
