...
class Client(datastore.Client[Config]):
    ...
    async def initialize_data(
        ...
        with self.__pool.connect() as conn:
            ...
            conn.execute(sqlalchemy.text("DROP TABLE IF EXISTS airports CASCADE"))
            # Create a new table
            conn.execute(
                sqlalchemy.text(
                    """
            ...
            # If the table already exists, drop it to avoid conflicts
            conn.execute(sqlalchemy.text("DROP TABLE IF EXISTS amenities CASCADE"))
            # Create a new table
            conn.execute(
                sqlalchemy.text(
                    """
            ...
            # If the table already exists, drop it to avoid conflicts
            conn.execute(sqlalchemy.text("DROP TABLE IF EXISTS flights CASCADE"))
            # Create a new table
            conn.execute(
                sqlalchemy.text(
                    """
